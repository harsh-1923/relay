// AUTO-GENERATED by scripts/build-icons.mjs — do not edit by hand.
import { createIcon } from '../createIcons';
import type { IconVariants } from '../types';

const arrowUp: IconVariants = {
  Stroke: [
    [
      'path',
      {
        d: 'M6 9.8304C7.55556 7.727 9.37278 5.83783 11.4057 4.20952C11.5801 4.06984 11.79 4 12 4M18 9.8304C16.4444 7.727 14.6272 5.83783 12.5943 4.20952C12.4199 4.06984 12.21 4 12 4M12 4V20',
        stroke: 'currentColor',
        key: 's0',
      },
    ],
  ],
  Solid: [
    [
      'path',
      {
        d: 'M17.95 10.8291L13 10.5813L13 20C13 20.5523 12.5523 21 12 21C11.4477 21 11 20.5523 11 20L11 10.5813L6.05001 10.8291C5.66391 10.8485 5.30125 10.6435 5.11862 10.3028C4.936 9.96208 4.96612 9.54661 5.19599 9.23579C6.80273 7.06319 8.68001 5.11149 10.7805 3.42903C11.136 3.1443 11.5675 3 12 3C12.4325 3 12.864 3.1443 13.2195 3.42903C15.32 5.11149 17.1973 7.06319 18.804 9.23579C19.0339 9.54661 19.064 9.96208 18.8814 10.3028C18.6988 10.6435 18.3361 10.8485 17.95 10.8291Z',
        fill: 'currentColor',
        key: 's0',
      },
    ],
  ],
  Contrast: [
    [
      'path',
      {
        d: 'M11.4057 4.20952C9.37278 5.83783 7.55556 7.727 6 9.8304L12 9.53L18 9.8304C16.4444 7.727 14.6272 5.83783 12.5943 4.20952C12.4199 4.06984 12.21 4 12 4C11.79 4 11.5801 4.06984 11.4057 4.20952Z',
        fill: 'currentColor',
        opacity: 0.28,
        key: 'c0',
      },
    ],
    [
      'path',
      {
        d: 'M12 9.53L6 9.8304C7.55556 7.727 9.37278 5.83783 11.4057 4.20952C11.5801 4.06984 11.79 4 12 4C12.21 4 12.4199 4.06984 12.5943 4.20952C14.6272 5.83783 16.4444 7.727 18 9.8304L12 9.53ZM12 9.53L12 20',
        stroke: 'currentColor',
        key: 'c1',
      },
    ],
  ],
  'Duo Stroke': [
    ['path', { d: 'M12 4V20', stroke: 'currentColor', opacity: 0.28, key: 'd0' }],
    [
      'path',
      {
        d: 'M6 9.8304C7.55556 7.727 9.37278 5.83783 11.4057 4.20952C11.5801 4.06984 11.79 4 12 4C12.21 4 12.4199 4.06984 12.5943 4.20952C14.6272 5.83783 16.4444 7.727 18 9.8304',
        stroke: 'currentColor',
        key: 'd1',
      },
    ],
  ],
  'Duo Solid': [
    ['path', { d: 'M12 5V21', stroke: 'currentColor', opacity: 0.28, key: 'd0' }],
    [
      'path',
      {
        d: 'M12 4C11.5675 4 11.136 4.1443 10.7805 4.42903C8.68001 6.11149 6.80273 8.06319 5.19599 10.2358C4.97149 10.5394 4.9371 10.9435 5.10709 11.2806C5.27707 11.6178 5.62244 11.8304 6.00001 11.8304L18 11.8304C18.3776 11.8304 18.7229 11.6177 18.8929 11.2806C19.0629 10.9435 19.0285 10.5394 18.804 10.2358C17.1973 8.06319 15.32 6.11149 13.2195 4.42903C12.864 4.1443 12.4325 4 12 4Z',
        fill: 'currentColor',
        key: 'd1',
      },
    ],
  ],
};

export const ArrowUp = createIcon('arrow-up', arrowUp);
export default ArrowUp;
