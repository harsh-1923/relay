// AUTO-GENERATED by scripts/build-icons.mjs — do not edit by hand.
import { createIcon } from '../createIcons';
import type { IconVariants } from '../types';

const listCheckBox: IconVariants = {
  Stroke: [
    [
      'path',
      {
        d: 'M12 12H21M12 18H21M12 6L21 6M3 16.1047L4.89736 18C5.80992 16.4043 7.07282 15.0366 8.59087 14M4 10H7C7.55228 10 8 9.55228 8 9V6C8 5.44772 7.55228 5 7 5H4C3.44772 5 3 5.44772 3 6V9C3 9.55228 3.44772 10 4 10Z',
        stroke: 'currentColor',
        key: 's0',
      },
    ],
  ],
  Solid: [
    [
      'path',
      {
        d: 'M4 4C2.89543 4 2 4.89543 2 6V9C2 10.1046 2.89543 11 4 11H7C8.10457 11 9 10.1046 9 9V6C9 4.89543 8.10457 4 7 4H4Z',
        fill: 'currentColor',
        key: 's0',
      },
    ],
    [
      'path',
      {
        d: 'M12 5C11.4477 5 11 5.44772 11 6C11 6.55228 11.4477 7 12 7L21 7C21.5523 7 22 6.55229 22 6C22 5.44772 21.5523 5 21 5H12Z',
        fill: 'currentColor',
        key: 's1',
      },
    ],
    [
      'path',
      {
        d: 'M12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13H21C21.5523 13 22 12.5523 22 12C22 11.4477 21.5523 11 21 11H12Z',
        fill: 'currentColor',
        key: 's2',
      },
    ],
    [
      'path',
      {
        d: 'M9.15479 14.8258C9.61088 14.5144 9.72814 13.8922 9.4167 13.4361C9.10525 12.98 8.48304 12.8627 8.02695 13.1742C6.74065 14.0525 5.6236 15.1505 4.72524 16.4146L3.70673 15.3972C3.316 15.0069 2.68283 15.0072 2.29252 15.3979C1.9022 15.7887 1.90254 16.4218 2.29327 16.8121L4.19063 18.7075C4.41102 18.9276 4.72142 19.0325 5.03018 18.9911C5.33894 18.9498 5.61078 18.7669 5.76543 18.4964C6.60284 17.0322 7.76175 15.7771 9.15479 14.8258Z',
        fill: 'currentColor',
        key: 's3',
      },
    ],
    [
      'path',
      {
        d: 'M12 17C11.4477 17 11 17.4477 11 18C11 18.5523 11.4477 19 12 19H21C21.5523 19 22 18.5523 22 18C22 17.4477 21.5523 17 21 17H12Z',
        fill: 'currentColor',
        key: 's4',
      },
    ],
  ],
  Contrast: [
    [
      'path',
      {
        d: 'M3 6C3 5.44772 3.44772 5 4 5H7C7.55228 5 8 5.44772 8 6V9C8 9.55228 7.55228 10 7 10H4C3.44772 10 3 9.55228 3 9V6Z',
        fill: 'currentColor',
        opacity: 0.28,
        key: 'c0',
      },
    ],
    [
      'path',
      {
        d: 'M12 12H21M12 18H21M12 6L21 6M3 16.1047L4.89736 18C5.80992 16.4043 7.07282 15.0366 8.59087 14M4 10H7C7.55228 10 8 9.55228 8 9V6C8 5.44772 7.55228 5 7 5H4C3.44772 5 3 5.44772 3 6V9C3 9.55228 3.44772 10 4 10Z',
        stroke: 'currentColor',
        key: 'c1',
      },
    ],
  ],
  'Duo Stroke': [
    [
      'path',
      { d: 'M12 12H21M12 18H21M12 6L21 6', stroke: 'currentColor', opacity: 0.28, key: 'd0' },
    ],
    [
      'path',
      {
        d: 'M3 16.1047L4.89736 18C5.80992 16.4043 7.07282 15.0366 8.59087 14M4 10H7C7.55228 10 8 9.55228 8 9V6C8 5.44772 7.55228 5 7 5H4C3.44772 5 3 5.44772 3 6V9C3 9.55228 3.44772 10 4 10Z',
        stroke: 'currentColor',
        key: 'd1',
      },
    ],
  ],
  'Duo Solid': [
    [
      'path',
      { d: 'M12 12H21M12 18H21M12 6L21 6', stroke: 'currentColor', opacity: 0.28, key: 'd0' },
    ],
    [
      'path',
      {
        d: 'M4 4C2.89543 4 2 4.89543 2 6V9C2 10.1046 2.89543 11 4 11H7C8.10457 11 9 10.1046 9 9V6C9 4.89543 8.10457 4 7 4H4Z',
        fill: 'currentColor',
        key: 'd1',
      },
    ],
    [
      'path',
      {
        d: 'M9.15479 14.8258C9.61088 14.5144 9.72814 13.8922 9.4167 13.4361C9.10525 12.98 8.48304 12.8627 8.02695 13.1742C6.74065 14.0525 5.6236 15.1505 4.72524 16.4146L3.70673 15.3972C3.316 15.0069 2.68283 15.0072 2.29252 15.3979C1.9022 15.7887 1.90254 16.4218 2.29327 16.8121L4.19063 18.7075C4.41102 18.9276 4.72142 19.0325 5.03018 18.9911C5.33894 18.9498 5.61078 18.7669 5.76543 18.4964C6.60284 17.0322 7.76175 15.7771 9.15479 14.8258Z',
        fill: 'currentColor',
        key: 'd2',
      },
    ],
  ],
};

export const ListCheckBox = createIcon('list-check-box', listCheckBox);
export default ListCheckBox;
