 ...
 *        ▼
 *   src/icons/<icon-name>.ts      (IconVariants + createIcon call)
...
function loadManifestMeta() {
  try {
    ...
    return new Map(
      m.map((i) => [i.slug, { section: i.section || 'Other', category: i.category || 'other' }]),
    );
  } catch {
    return new Map();
  }
...
const serializeNode = ([tag, attrs]) => `["${tag}", ${JSON.stringify(attrs)}]`;
...
export const ${comp} = createIcon("${name}", ${local});
export default ${comp};
`;
...
function writeMeta(comps) {
  ...
  const out = `// AUTO-GENERATED by scripts/build-icons.mjs — do not edit by hand.
/**
 * \`component\`: exported component name, e.g. "HomeAi" (key into the package's named exports).
 * \`name\`: kebab-case icon id, e.g. "home-ai".
 * \`section\`: human-readable Figma section, e.g. "Ai", "Web3 & Crypto".
 * \`category\`: slugified section, e.g. "ai", "web3-crypto".
 */
...
function main() {
  ...
  const barrel =
    comps.map(({ name, comp }) => `export { ${comp} } from "./icons/${name}";`).join('\n') +
